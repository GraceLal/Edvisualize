...
let masspopChart = new Chart(myChart, {
  type: "doughnut", //bar chart,pie,line,donut,radar,polarArea,horizontalBar
  data: {
    labels: [
      "Mumbai",
      "Delhi",
...
function removeData(chart) {
  let newCity = document.getElementById("city").value;
  let cityIndex = chart.data.labels.findIndex(city => city === newCity);
  chart.data.datasets[0].data = chart.data.datasets[0].data.filter(
    (data) => data !== chart.data.datasets[0].data[cityIndex]
  );
